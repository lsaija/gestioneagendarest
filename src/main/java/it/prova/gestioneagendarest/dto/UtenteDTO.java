...
import it.prova.gestioneagendarest.model.StatoUtente;
...
public class UtenteDTO {
	...
	public Utente buildUtenteModelConAgende(Utente utenteModel,boolean includeIdRoles,boolean includeAgende) {
		Utente result = new Utente(this.id, this.username, this.password, this.nome, this.cognome, this.email,
				this.dateCreated, this.stato);
		if (includeIdRoles && ruoliIds != null)
			result.setRuoli(Arrays.asList(ruoliIds).stream().map(id -> new Ruolo(id)).collect(Collectors.toSet()));
		if (includeAgende && agendeIds != null)
	...
	// niente password...
	public static UtenteDTO buildUtenteDTOFromModelConAgende(Utente utenteModel,boolean includeAgende) {
		UtenteDTO result = new UtenteDTO(utenteModel.getId(), utenteModel.getUsername(), utenteModel.getNome(),
				utenteModel.getCognome(), utenteModel.getStato());
